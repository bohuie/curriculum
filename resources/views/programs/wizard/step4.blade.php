                        ...
                        <div class="card-body">
                            <nav class="mt-2">
                                <div class="nav nav-tabs justify-content-center" id="nav-tab" role="tablist">
                                    ...
                                    <button class="nav-link w-25" id="nav-non-required-tab" data-bs-toggle="tab" data-bs-target="#nav-non-required" type="button" role="tab" aria-controls="nav-non-required" aria-selected="false">Non-Required Courses</button>
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                ...
                                <div class="tab-pane fade" id="nav-non-required" role="tabpanel" aria-labelledby="nav-non-required-tab">
                                    <!-- Non-Required Courses frequency distribution table -->
                                    <div class="card-body">
                                        <h5 class="card-title">
                                            Curriculum Map
                                        </h5>
                                        @if( count($nonRequiredProgramCourses) < 1 )
                                            <div class="alert alert-warning wizard">
                                                <i class="bi bi-exclamation-circle-fill pr-2 fs-5"></i>There are no first year courses set for this program yet.                   
                                            </div>
                                        @elseif ($ploCount < 1) 
                                        ...
                                        @else
                                            ...
                                            <table class="table table-bordered table-sm" style="width: 95%; margin:auto; table-layout: fixed; border: 1px solid white; color: black;">
                                                ...
                                                <!-- Show all courses associated to the program -->
                                                @foreach($nonRequiredProgramCourses as $course)
                                                    <tr>
                                                        <th colspan="1" style="background-color: rgba(0, 0, 0, 0.03);">
                                                        {{$course->course_code}} {{$course->course_num}} {{$course->section}}
                                                        <br>
                                                        ...
                                                        @foreach($ploProgramCategories as $index => $plo)
                                                            @if ($plo->plo_category != NULL)
                                                            <!-- Check if ['pl_outcome_id']['course_id'] are in the array -->
                                                                @if(isset($storeNonRequired[$plo->pl_outcome_id][$course->course_id]))
                                                                    <!-- Check if a Tie is present -->
                                                                    @if(isset($storeNonRequired[$plo->pl_outcome_id][$course->course_id]['map_scale_id_tie']))
                                                                        <td class="text-center align-middle" style="background:repeating-linear-gradient(45deg, transparent, transparent 8px, #ccc 8px, #ccc 16px), linear-gradient( to bottom, #fff, #999);" data-toggle="tooltip" data-html="true" data-bs-placement="right" title="@foreach($storeNonRequired[$plo->pl_outcome_id][$course->course_id]['frequencies'] as $index => $freq) {{$index}}: {{$freq}}<br> @endforeach">
                                                                            <span style="color: black;">
                                                                                {{$storeNonRequired[$plo->pl_outcome_id][$course->course_id]['map_scale_abv']}}
                                                                            </span>
                                                                        </td>
                                                                    @else
                                                                        <td class="text-center align-middle" style="background-color: {{ $storeNonRequired[$plo->pl_outcome_id][$course->course_id]['colour'] }};" data-toggle="tooltip" data-html="true" data-bs-placement="right" title="@foreach($storeNonRequired[$plo->pl_outcome_id][$course->course_id]['frequencies'] as $index => $freq) {{$index}}: {{$freq}}<br> @endforeach">
                                                                            <span style="color: black;">
                                                                                {{$storeNonRequired[$plo->pl_outcome_id][$course->course_id]['map_scale_abv']}}
                                                                            </span>
                                                                        </td>
                                                        ...
                                                        @foreach($plos as $plo)
                                                            @if ($plo->plo_category == NULL)
                                                                ...
                                                                @if(isset($storeNonRequired[$plo->pl_outcome_id][$course->course_id]))
                                                                    <!-- Check if a Tie is present -->
                                                                    @if(isset($storeNonRequired[$plo->pl_outcome_id][$course->course_id]['map_scale_id_tie']))
                                                                        <td class="text-center align-middle" style="background:repeating-linear-gradient( 45deg, transparent, transparent 10px, #ccc 10px, #ccc 20px), linear-gradient( to bottom, #eee, #999);" data-toggle="tooltip" data-html="true" data-bs-placement="right" title="@foreach($storeNonRequired[$plo->pl_outcome_id][$course->course_id]['frequencies'] as $index => $freq) {{$index}}: {{$freq}}<br> @endforeach">
                                                                            <span style="color: black;">
                                                                                {{$storeNonRequired[$plo->pl_outcome_id][$course->course_id]['map_scale_abv']}}
                                                                            </span>
                                                                        </td>
